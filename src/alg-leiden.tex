...
\Function{leiden}{$G$} \label{alg:leiden--begin}
  ...
  \ForAll{$l_p \in [0 .. \text{\small{MAX\_PASSES}})$} \label{alg:leiden--passes-begin}
    ...
    \State $l_i \gets leidenMove(G', C', K', \Sigma')$ \label{alg:leiden--local-move}
    \If{$l_i \le 1$} \textbf{break} \Comment{Globally converged?} \label{alg:leiden--globally-converged}
    \EndIf
